...
function max_format_admin_notice(array $incoming, string $text, array $update): string
{
    ...
    if ($chatId) {
        $lines[] = '/max ' . $chatId . ' Здравствуйте, Андрей. Получили вашу заявку. Можете прислать фото/документы?';
    } else {
        $lines[] = 'MAX Chat ID не найден, ответить командой /max пока нельзя.';
    }
...
if ($hasEnoughData) {
    $replyText = "Спасибо. Заявку получил и передал в рабочую группу A&A Asset Team.\n\nДля связи мы используем этот чат в MAX. Если есть фото, документы или ссылка на объявление — отправьте их следующим сообщением.";
} else {
