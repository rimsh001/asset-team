(() => {
  ...
  if (!$('#brand-logo-style')) {
    ...
    style.textContent = `
      ...
      a[href*="contact-max"],.header-messenger-btn--max,.page-link--max{display:none!important}
      ...
      .page-hero h1{font-size:clamp(42px,6vw,72px);max-width:920px;margin:18px 0 24px}.page-hero p{font-size:20px;color:rgba(255,255,255,.76);max-width:820px}.page-links{display:flex;flex-wrap:wrap;gap:12px;margin-top:30px}.page-link{display:inline-flex;align-items:center;border:1px solid rgba(255,255,255,.15);border-radius:999px;padding:11px 15px;color:#fff;background:rgba(255,255,255,.05);font-weight:700}.page-link--telegram{background:rgba(42,171,238,.15);border-color:rgba(42,171,238,.32)}.asset-nav{display:grid;grid-template-columns:repeat(3,1fr);gap:16px}.asset-nav a{background:var(--paper);border:1px solid var(--line);border-radius:20px;padding:22px}.asset-nav span{font-family:var(--mono);font-size:12px;color:var(--gold);font-weight:700}.asset-nav h3{font-size:23px;margin:12px 0 8px}.asset-nav p{color:var(--muted)}
  ...
  const enhanceTelegramLinks = () => {
    $$('a[href*="contact-telegram"]').forEach((link) => {
      addTelegramIcon(link);
